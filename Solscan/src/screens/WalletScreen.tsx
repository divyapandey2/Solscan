...
// ============================================
// The URL for the official Solana public mainnet JSON-RPC node
const RPC = "https://api.mainnet-beta.solana.com";

//Fires a standard HTTP POST request containing JSON-RPC formatted data to the Solana blockchain node.
const rpc = async (method: string, params: any[]) => {
    ...
    //Converts the response back to JSON. If the blockchain returns an error object, it throws an error. Otherwise, it returns the result data.
    const json = await res.json();
    if (json.error) throw new Error(json.error.message);
    return json.result;
};
//Sends the getBalance query for the target wallet. It returns the balance in lamports (similar to satoshis/wei). We divide by 1,000,000,000 to convert it to user-facing SOL.
const getBalance = async (addr: string) => {
    const result = await rpc("getBalance", [addr]);
    return result.value / 1_000_000_000;
};
//Fetches all SPL token accounts (like USDC, Bonk, etc.) owned by the address using the standard Solana Token Program.
...
//Fetches the most recent transaction signatures for the wallet address.
...
// A string formatter that cuts down a long 44-character Solana address to show just the beginning and end
